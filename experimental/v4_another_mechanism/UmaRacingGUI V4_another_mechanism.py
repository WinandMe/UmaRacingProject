...
# === New enums for engine phases/mood/lane ===
class Phase(Enum):
    START = auto()
    MIDDLE = auto()
    FINAL_CORNER = auto()
...
# === New enums for advanced mechanics ===
...
class UmaRacingGUI(tk.Tk):
    def __init__(self):
        ...
        # === JP-engine runtime state per horse ===
        self.horse_v = {}          # current speed (m/s)
        self.horse_a = {}          # current acceleration (m/s^2)
        self.horse_phase = {}      # Phase enum
        self.horse_block_front = {}
        self.horse_block_side = {}
        self.horse_lane = {}       # integer lane index
        self.horse_last_spurt = {}
        self.horse_collapsed = {}
        self.horse_mood = {}       # Mood enum
        self.horse_start_delay = {}# start jitter seconds

        # === Advanced race mechanics ===
        self.track_condition = TrackCondition.FIRM
        self.weather = Weather.CLEAR
        self.race_grade = RaceGrade.G1
        self.course_direction = "clockwise"
        self.num_corners = 4
        self.home_straight_length = 400
        self.track_bias = 0.0
        
        # Running style position targets
        self.style_position_targets = {
            'FR': (1, 3),
            'PC': (2, 5),
            'LS': (4, 8),
            'EC': (9, 12)
        ...
        # Advanced horse states
        self.horse_form = {}
        self.horse_motivation = {}
        self.horse_equipment = {}
        
        # Cornering mechanics
        self.corner_positions = []
        self.horse_cornering_skill = {}
        
        # Drafting mechanics
        self.drafting_bonus = 0.03
        self.drafting_distance = 2.0
    ...
    def load_skill_effects(self):
        """Load skill effects from UmaSkills.json and convert to simulation format"""
        try:
            skill_file_path = os.path.join(os.path.dirname(__file__), 'UmaSkills.json')
            with open(skill_file_path, 'r', encoding='utf-8') as f:
                raw_skills = json.load(f)

            # Convert skills to simulation format
            self.skill_effects = {}
            if isinstance(raw_skills, dict):
                # If it's a dict, process as before
                for skill_name, skill_data in raw_skills.items():
                    # Convert trigger to phase
                    trigger = skill_data.get('trigger', 'mid')
                    phase_mapping = {
                        'start': 'start',
                        'opening': 'start',
                        'early': 'start',
                    ...
                    phase = phase_mapping.get(trigger, 'mid')

                    # Convert magnitude_type to effect multiplier
                    magnitude = skill_data.get('magnitude_type', 'moderate')
                    ...
                    # Convert chance to 0-1
                    chance = min(skill_data.get('chance', 1.0) / 100.0, 1.0) if skill_data.get('chance', 1.0) > 1 else skill_data.get('chance', 1.0)

                    # Determine effect type based on skill type
                    skill_type = skill_data.get('type', 'speed')
                    effect_type = 'momentum_boost'  # default

                    if 'stamina' in skill_type:
                        effect_type = 'stamina_recovery'
                    elif 'speed' in skill_type or 'acceleration' in skill_type:
                        effect_type = 'momentum_boost'
                    # Add more effect types as needed

                    # Calculate value based on magnitude
                    base_value = skill_data.get('value', 0.08)
                    value = base_value * multiplier

                    self.skill_effects[skill_name] = {
                        'type': effect_type,
                        'value': value,
                        'duration': skill_data.get('duration', 3.0),
                        'cooldown': 30.0,  # Default cooldown
                        'phase': phase,
                        'chance': chance
                    }
            elif isinstance(raw_skills, list):
                # If it's a list of skill names, create default effects
            ...
            print(f"Loaded and converted {len(self.skill_effects)} skill effects from {skill_file_path}")
        except Exception as e:
            print(f"Error loading skill effects: {e}")
            self.skill_effects = {}
    ...
    def draw_track(self):
        """Draw Uma Musume style race track"""
    ...
    def draw_distance_marker(self, marker_distance, race_distance):
        ...
        # position X from remaining marker
    ...
    def prepare_real_time_simulation(self, config_data):
        ...
        for uma in umas:
            ...
            # Initialize skills for this uma
            uma_skills = {}
            if 'skills' in uma:
                for skill_name in uma['skills']:
                    if skill_name in self.skill_effects:
                        uma_skills[skill_name] = {
                            'active': False,
                            'last_activation': -1000,  # Start with cooldown
                            'duration_left': 0,
                            'effect': None
                        }
    ...
    def init_engine_runtime(self):
        for name in self.sim_data.get('uma_stats', {}).keys():
            self.horse_v[name] = 0.0
            self.horse_a[name] = 0.0
            self.horse_phase[name] = Phase.START
            self.horse_block_front[name] = False
            self.horse_block_side[name] = False
            self.horse_lane[name] = 0
            self.horse_last_spurt[name] = False
            self.horse_collapsed[name] = False
            # Random mood: 70% normal, 20% good, 10% bad
            mood_weights = [0.7, 0.2, 0.1]
            mood_choices = [Mood.NORMAL, Mood.GOOD, Mood.BAD]
            self.horse_mood[name] = random.choices(mood_choices, weights=mood_weights)[0]
            self.horse_start_delay[name] = random.uniform(0.0, 0.35)

    def seed_lanes_from_styles(self):
        # Assign rough initial lanes by style
        for name, stats in self.sim_data.get('uma_stats', {}).items():
            style = stats['running_style']
            if style == 'FR':
                self.horse_lane[name] = 0
            elif style == 'PC':
                self.horse_lane[name] = 1
            elif style == 'LS':
                self.horse_lane[name] = 2
            else:  # EC
                self.horse_lane[name] = 3

    def update_phase_engine(self, name, race_distance):
        x = self.horse_distances[name]
        race_type = self.sim_data.get('race_type', 'Medium')

        # Phase timings based on race type
        phase_thresholds = {
            'Sprint': {'start_end': 0.20, 'middle_end': 0.70, 'final_start': 0.70, 'last_spurt_start': 0.90},
            'Mile': {'start_end': 0.15, 'middle_end': 0.60, 'final_start': 0.60, 'last_spurt_start': 0.85},
            'Medium': {'start_end': 0.10, 'middle_end': 0.50, 'final_start': 0.50, 'last_spurt_start': 0.80},
            'Long': {'start_end': 0.05, 'middle_end': 0.40, 'final_start': 0.40, 'last_spurt_start': 0.70}
        }

        thresholds = phase_thresholds.get(race_type, phase_thresholds['Medium'])
        progress = x / race_distance

        if progress < thresholds['start_end']:
            self.horse_phase[name] = Phase.START
        elif progress < thresholds['middle_end']:
            self.horse_phase[name] = Phase.MIDDLE
        elif progress < thresholds['last_spurt_start']:
            self.horse_phase[name] = Phase.FINAL_CORNER
        else:
            self.horse_phase[name] = Phase.LAST_SPURT

    def is_extra_move_lane(self, name):
        return self.horse_phase[name] == Phase.FINAL_CORNER

    def target_speed_cap(self, name, uma_stat):
        mood_scale = self.horse_mood[name].value
        base = uma_stat['top_speed']
        phase = self.horse_phase[name]
        if phase == Phase.START:
            cap = 0.75 * base
        elif phase == Phase.MIDDLE:
            cap = 0.90 * base
        elif phase == Phase.FINAL_CORNER:
            cap = 0.95 * base
        else:
            cap = 1.00 * base
        cap *= random.uniform(0.98, 1.02)
        if self.horse_block_front[name] and not self.is_extra_move_lane(name):
            cap *= 0.94
        return cap * uma_stat['base_performance'] * mood_scale

    def base_accel_engine(self, name, uma_stat):
        a = 0.004 * uma_stat['power']
        if self.horse_block_front[name]:
            a *= 0.5
        if self.horse_block_side[name]:
            a *= 0.7
        return a

    def required_last_spurt_stamina(self, name, uma_stat, race_distance):
        remaining = max(race_distance - self.horse_distances[name], 0.0)
        intended_v = uma_stat['top_speed'] * self.horse_mood[name].value
        base_drag = {'Sprint': 0.9, 'Mile': 1.0, 'Medium': 1.05, 'Long': 1.1}.get(uma_stat['race_type'], 1.05)
        guts_factor = 1.0 - min(uma_stat['guts'] / 1200.0, 0.20)
        return remaining * intended_v * base_drag * guts_factor * 0.002

    def check_last_spurt_gate(self, name, uma_stat, race_distance):
        if self.horse_phase[name] == Phase.LAST_SPURT and not (self.horse_last_spurt[name] or self.horse_collapsed[name]):
            need = self.required_last_spurt_stamina(name, uma_stat, race_distance)
            if self.horse_stamina[name] >= need:
                self.horse_last_spurt[name] = True
            else:
                self.horse_collapsed[name] = True
                self.horse_v[name] = min(self.horse_v[name], 0.85 * uma_stat['top_speed'] * self.horse_mood[name].value)

    def stamina_drain_engine(self, name, uma_stat):
        phase = self.horse_phase[name]
        v = self.horse_v[name]
        corner_weight = 1.0 + (0.25 if phase == Phase.FINAL_CORNER else 0.0)
        collapse_tax = 1.3 if self.horse_collapsed[name] else 1.0
        base_drag = {'Sprint': 0.85, 'Mile': 0.95, 'Medium': 1.0, 'Long': 1.1}.get(uma_stat['race_type'], 1.0)
        guts_factor = 1.0 - min(uma_stat['guts'] / 1200.0, 0.15)
        drain = (v * 0.12) * base_drag * corner_weight * collapse_tax * guts_factor
        drain *= random.uniform(0.95, 1.05)
        return drain

    def apply_stamina_engine(self, name, uma_stat, dt):
        d = self.stamina_drain_engine(name, uma_stat) * dt
        self.horse_stamina[name] = max(self.horse_stamina[name] - d, 0.0)
        if self.horse_stamina[name] <= 0.0:
            self.horse_a[name] *= 0.5
            self.horse_v[name] *= 0.98

    def sense_blocking_engine(self, name, frame_positions, radius=2.0):
        my_x = next((d for n, d in frame_positions if n == name), self.horse_distances[name])
        front = False
        side = False
        my_lane = self.horse_lane[name]
        for other, other_x in frame_positions:
            if other == name:
                continue
            same_lane = (self.horse_lane.get(other, 0) == my_lane)
            side_lane = (abs(self.horse_lane.get(other, 0) - my_lane) == 1)
            if same_lane and 0.0 < (other_x - my_x) < radius:
                front = True
            if side_lane and abs(other_x - my_x) < 1.0:
                side = True
        self.horse_block_front[name] = front
        self.horse_block_side[name] = side

    def lane_change_success_engine(self, name, uma_stat):
        if self.horse_block_side[name]:
            return False
        base = 0.35 + 0.0004 * uma_stat['power'] + 0.0003 * uma_stat['wisdom']
        if self.is_extra_move_lane(name):
            base += 0.05
        return random.random() < min(max(base, 0.1), 0.9)

    def lane_ai_engine(self, name, uma_stat, frame_positions):
        if self.horse_phase[name] == Phase.FINAL_CORNER:
            mode = LaneMode.EXTRA_MOVE
        elif self.horse_block_front[name]:
            mode = LaneMode.OVERTAKE
        else:
            mode = LaneMode.NORMAL

        if mode in (LaneMode.OVERTAKE, LaneMode.EXTRA_MOVE):
            if self.lane_change_success_engine(name, uma_stat):
                lane_left = self.horse_lane[name] - 1
                lane_right = self.horse_lane[name] + 1
                left_density = sum(1 for n, _ in frame_positions if self.horse_lane.get(n, 0) == lane_left)
                right_density = sum(1 for n, _ in frame_positions if self.horse_lane.get(n, 0) == lane_right)
                if right_density < left_density:
                    self.horse_lane[name] = lane_right
                else:
                    self.horse_lane[name] = lane_left
            else:
                self.horse_v[name] *= 0.99  # failed change small penalty

    # === Advanced race mechanics ===
    def initialize_advanced_mechanics(self):
        """Initialize advanced race mechanics"""
        if not self.sim_data:
            return
            
        uma_stats = self.sim_data.get('uma_stats', {})
        
        # Initialize advanced states
        for name in uma_stats.keys():
            # Random form between 0.9-1.1
            self.horse_form[name] = random.uniform(0.9, 1.1)
            # Random motivation between 0.95-1.05  
            self.horse_motivation[name] = random.uniform(0.95, 1.05)
            # Basic equipment
            self.horse_equipment[name] = {"shoes": "standard", "bonus": 1.0}
            # Cornering skill based on wisdom
            wisdom = uma_stats[name].get('wisdom', 600)
            self.horse_cornering_skill[name] = 0.5 + (wisdom / 1200.0) * 0.5

    def generate_corner_positions(self, race_distance):
        """Generate corner positions based on race distance"""
        self.corner_positions = []
        
        if race_distance <= 1400:  # Sprint
            corners = [(400, 600), (900, 1100)]
        elif race_distance <= 2000:  # Mile
            corners = [(300, 500), (800, 1000), (1300, 1500)]
        else:  # Medium/Long
            corners = [(400, 600), (1000, 1200), (1600, 1800), (2200, 2400)]
        
        self.corner_positions = corners

    def calculate_corner_effects(self, name, current_distance, race_distance):
        """Calculate speed reduction in corners based on position and skill"""
        if not self.corner_positions:
            self.generate_corner_positions(race_distance)
        
        in_corner = False
        corner_severity = 0.0
        
        for corner_start, corner_end in self.corner_positions:
            if corner_start <= current_distance <= corner_end:
                in_corner = True
                # Corner severity based on position in pack
                position = self.get_horse_position(name)
                if position <= 3:  # Inside positions have advantage
                    corner_severity = 0.92  # 8% reduction
                else:  # Outside positions suffer more
                    corner_severity = 0.85  # 15% reduction
                break
        
        if in_corner:
            # Apply cornering skill to reduce penalty
            skill_bonus = self.horse_cornering_skill[name]
            final_severity = corner_severity + (1.0 - corner_severity) * skill_bonus
            return final_severity
        
        return 1.0  # No reduction on straights

    def get_horse_position(self, name):
        """Get current race position for a horse"""
        positions = sorted([(n, d) for n, d in self.horse_distances.items() 
                           if not self.horse_finished[n] and not self.horse_dnf[n]['dnf']], 
                          key=lambda x: x[1], reverse=True)
        
        for i, (n, d) in enumerate(positions):
            if n == name:
                return i + 1
        return len(positions) + 1

    def calculate_drafting_effect(self, name, frame_positions):
        """Calculate drafting/slipstream bonus"""
        my_pos = self.get_horse_position(name)
        if my_pos == 1:  # Leader gets no draft
            return 1.0
            
        # Find horse directly in front
        front_horse = None
        front_distance = float('inf')
        my_x = self.horse_distances[name]
        
        for other_name, other_x in frame_positions:
            if other_name == name:
                continue
            if other_x > my_x and (other_x - my_x) < front_distance:
                front_distance = other_x - my_x
                front_horse = other_name
        
        if front_horse and front_distance <= self.drafting_distance:
            return 1.0 + self.drafting_bonus
        
        return 1.0

    def calculate_position_penalty_bonus(self, name):
        """Calculate bonus/penalty based on running style position target"""
        current_pos = self.get_horse_position(name)
        uma_stat = self.sim_data['uma_stats'][name]
        running_style = uma_stat['running_style']
        
        target_min, target_max = self.style_position_targets.get(running_style, (1, 12))
        
        if target_min <= current_pos <= target_max:
            return 1.02  # 2% bonus in ideal position
        elif current_pos < target_min:
            return 0.98  # 2% penalty for being too far forward
        else:
            return 0.96  # 4% penalty for being too far back

    def apply_environmental_effects(self, name, uma_stat):
        """Apply weather, track condition, and race grade effects"""
        # Track condition effect
        track_multiplier = self.track_condition.value
        
        # Weather effect
        weather_multiplier = self.weather.value
        
        # Race grade pressure effect
        grade_multiplier = self.race_grade.value
        
        # Surface aptitude effect
        surface = self.sim_data.get('race_surface', 'Turf')
        surface_apt = uma_stat.get('surface_aptitude', 'B')
        apt_multipliers = {'S': 1.05, 'A': 1.02, 'B': 1.0, 'C': 0.98, 'D': 0.95, 'E': 0.92, 'F': 0.88, 'G': 0.85}
        surface_multiplier = apt_multipliers.get(surface_apt, 1.0)
        
        # Form and motivation
        form_multiplier = self.horse_form.get(name, 1.0)
        motivation_multiplier = self.horse_motivation.get(name, 1.0)
        
        # Equipment bonus
        equipment_multiplier = self.horse_equipment.get(name, {}).get('bonus', 1.0)
        
        total_multiplier = (track_multiplier * weather_multiplier * grade_multiplier * 
                           surface_multiplier * form_multiplier * motivation_multiplier * 
                           equipment_multiplier)
        
        return total_multiplier

    def update_advanced_mechanics(self, name, uma_stat, frame_positions, current_distance, race_distance, dt):
        """Update all advanced mechanics in one call"""
        # Environmental effects
        env_multiplier = self.apply_environmental_effects(name, uma_stat)
        
        # Cornering effects
        corner_multiplier = self.calculate_corner_effects(name, current_distance, race_distance)
        
        # Drafting effects
        draft_multiplier = self.calculate_drafting_effect(name, frame_positions)
        
        # Position strategy effects
        position_multiplier = self.calculate_position_penalty_bonus(name)
        
        # Apply all multipliers to speed
        total_multiplier = env_multiplier * corner_multiplier * draft_multiplier * position_multiplier
    ...
    def initialize_real_time_simulation(self):
        ...
        # Initialize skills
        self.horse_skills = {}
        for name, uma_stat in uma_stats.items():
            self.horse_skills[name] = {}
            for skill_name, skill_data in uma_stat.get('skills', {}).items():
                self.horse_skills[name][skill_name] = skill_data.copy()
        ...
        # Initialize JP-engine state
        self.init_engine_runtime()
        self.seed_lanes_from_styles()
        
        # Initialize advanced mechanics
        self.initialize_advanced_mechanics()
    ...
    # === Wisdom gate helper
    def wisdom_gate(self, uma_stat, base_prob):
        p = base_prob + min(uma_stat['wisdom'] / 1200.0, 0.2)
        return random.random() < min(p, 0.95)

    def calculate_real_time_positions(self, time_delta):
        """Calculate new positions using JP-style engine with advanced mechanics"""
        race_distance = self.sim_data.get('race_distance', 2500)
        race_type = self.sim_data.get('race_type', 'Medium')
        uma_stats = self.sim_data.get('uma_stats', {})

        frame_positions = []
        # assemble current positions for blocking sense
        for name in uma_stats.keys():
            frame_positions.append((name, self.horse_distances.get(name, 0.0)))

        # Initialize advanced mechanics on first call
        if not self.horse_form:
            self.initialize_advanced_mechanics()
        ...
        for uma_name in uma_stats.keys():
            ...
            if self.horse_incidents[uma_name]['type']:
                ...
                if incident_time >= self.horse_incidents[uma_name]['duration']:
                    ...
                    self.update_phase_engine(uma_name, race_distance)
                    v_cap = self.target_speed_cap(uma_name, uma_stat)
                    dv = 0.0
                    self.horse_v[uma_name] = min(self.horse_v[uma_name] + dv, v_cap) * speed_multiplier
                    self.apply_stamina_engine(uma_name, uma_stat, time_delta)
                    self.horse_distances[uma_name] += self.horse_v[uma_name] * time_delta
            ...
            # Apply advanced mechanics
            advanced_multiplier = self.update_advanced_mechanics(
                uma_name, uma_stat, frame_positions, current_distance, race_distance, time_delta
            )

            # Skills
            self.check_and_activate_skills(uma_name, uma_stat, race_distance, race_type)

            # Start delay gate
            if self.sim_time < self.horse_start_delay[uma_name]:
                continue

            # Phase, blocking, lane AI
            self.update_phase_engine(uma_name, race_distance)
            self.sense_blocking_engine(uma_name, frame_positions, radius=2.0)
            self.lane_ai_engine(uma_name, uma_stat, frame_positions)

            # Last-spurt gate
            self.check_last_spurt_gate(uma_name, uma_stat, race_distance)

            # Accel
            self.horse_a[uma_name] = self.base_accel_engine(uma_name, uma_stat)
            if self.horse_last_spurt[uma_name] and not self.horse_collapsed[uma_name]:
                self.horse_a[uma_name] *= 1.15
            elif self.horse_collapsed[uma_name]:
                self.horse_a[uma_name] *= 0.6

            # Apply advanced multiplier to acceleration
            self.horse_a[uma_name] *= advanced_multiplier

            # Cap and integrate
            v_cap = self.target_speed_cap(uma_name, uma_stat)
            self.horse_v[uma_name] = min(self.horse_v[uma_name] + self.horse_a[uma_name] * time_delta, v_cap)

            # Stamina drain tied to velocity/phase
            self.apply_stamina_engine(uma_name, uma_stat, time_delta)
        ...
        frame_positions = [(n, self.horse_distances.get(n, 0.0)) for n in uma_stats.keys()]
    ...
    def check_and_activate_skills(self, uma_name, uma_stat, race_distance, race_type):
        """Check and activate skills based on race phase, cooldown, chance, wisdom gate"""
        current_distance = self.horse_distances[uma_name]
        race_progress = current_distance / race_distance

        # Determine current phase (distance-based, your original mapping)
        if race_type == 'Sprint':
        ...
        for skill_name in self.horse_skills[uma_name]:
            ...
            # Wisdom-gated activation
            if not self.wisdom_gate(uma_stat, skill_effect.get('chance', 0.5)):
                continue
    ...
    def calculate_current_speed(self, uma_name, uma_stat, race_distance, race_type):
        """Your original speed calculator (used for UI label only)."""
        current_distance = self.horse_distances[uma_name]
        race_progress = current_distance / race_distance
    ...
    def update_fatigue_and_stamina(self, uma_name, uma_stat, race_progress, current_phase):
        """Original per-phase fatigue/stamina updates (coexists with engine drain)."""
        fatigue_rates = {
    ...
    def get_enhanced_commentary(self, current_time, positions, race_distance, remaining_distance, incidents, finished):
        """Enhanced commentary system with new mechanics"""
        commentaries = []
        ...
        # Add cornering commentary
        if remaining_distance in [800, 600, 400, 200] and random.random() < 0.3:
            corner_commentary = self.get_corner_commentary(positions)
            if corner_commentary:
                commentaries.append(corner_commentary)

        # Add drafting commentary
        if random.random() < 0.2 and len(commentaries) < 2:
            draft_commentary = self.get_drafting_commentary(positions)
            if draft_commentary:
                commentaries.append(draft_commentary)
    ...
    def get_corner_commentary(self, positions):
        """Commentary for cornering situations"""
        if not positions:
            return ""
        
        leader = positions[0][0]
        corner_phrases = [
            f"Into the turn! {leader} takes the ideal line!",
            f"Navigating the corner! Positioning is crucial here!",
            f"The field bunches up in the turn!",
            f"{leader} maintains the inside advantage through the corner!"
        ]
        return random.choice(corner_phrases)

    def get_drafting_commentary(self, positions):
        """Commentary for drafting situations"""
        if len(positions) < 2:
            return ""
        
        follower = positions[1][0]
        leader = positions[0][0]
        drafting_phrases = [
            f"{follower} tucks in behind {leader}, saving energy in the slipstream!",
            f"Smart running from {follower}, drafting behind the leader!",
            f"{follower} gets a tow from {leader}!"
        ]
        return random.choice(drafting_phrases)
    ...
    def update_display(self, frame_positions, race_distance):
        ...
        if frame_positions:
            ...
            # Display engine velocity for lead speed (optional: switch back to calculate_current_speed)
            current_speed = self.horse_v.get(leader_name, self.calculate_current_speed(leader_name, uma_stat, race_distance, self.sim_data['race_type']))
        ...
        # Position each horse with proper vertical spacing
        for name, (circle, number_text, name_text) in self.uma_icons.items():
            if name not in names_in_frame:
                continue
    ...
    def reset_simulation(self):
        ...
        # Clear advanced mechanics
        self.horse_form.clear()
        self.horse_motivation.clear()
        self.horse_equipment.clear()
        self.horse_cornering_skill.clear()
        self.corner_positions.clear()
