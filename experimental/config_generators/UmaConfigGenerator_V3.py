...
class UmaConfigGenerator:
    def __init__(self, root):
        ...
        self.skill_categories = {
            "Speed Skills": [
                "#Beauty Sprint", "Acceleration", "Speed Star", "Final Sprint",
                "Quick Start", "Rapid Acceleration", "Speed Demon", "Velocity Boost"
            ],
            "Stamina Skills": [
                "Second Wind", "Stamina Recovery", "Endurance", "Long Distance Expert",
                "Energy Conservation", "Breathing Technique", "Recovery Step", "Marathon Runner"
            ],
            "Power Skills": [
                "Power Surge", "Breakthrough", "All Out!", "Strength Boost",
                "Momentum Builder", "Forceful Run", "Power Drive", "Muscle Power"
            ],
            "Guts Skills": [
                "Guts Spirit", "Last Spurt", "Never Give Up", "Fighting Spirit",
                "Final Effort", "Determination", "Courage", "Resilience"
            ],
            "Wisdom Skills": [
                "#Cute Perturbation♪", "Wisdom Flash", "Corner Master", "Strategic Mind",
                "Tactical Run", "Smart Positioning", "Race IQ", "Calculated Move"
            ],
            "Unique Skills": [
                "#Go Crazy♪", "#LookatCurren", "#WannaGetCurren'd?", "Golden Factor",
                "Unique Ability", "Final Corner", "Miracle Run", "White Breath"
            ],
            "Position Skills": [
                "Front Runner's Pride", "Pace Chaser's Insight", "Late Surger's Patience",
                "End Closer's Resolve", "Escape Artist", "Chaser's Spirit", "Leader's Aura"
            ],
            "Condition Skills": [
                "Rain Specialist", "Mud Specialist", "Firm Ground Expert", "Turf Master",
                "Dirt Demon", "Good Break", "Hill Climb", "Downhill Dash"
            ],
            "Mental Skills": [
                "Ice Cool", "Hot Blooded", "Calm Mind", "Pressure Handler",
                "Crowd Pleaser", "Fan Support", "Champion's Heart", "Underdog Spirit"
            ]
        }
        
        # Flatten skills list for combobox
        self.skills = []
    ...
    def setup_ui(self):
        ...
        # New race condition settings
        ttk.Label(race_frame, text="Track Condition:").grid(row=2, column=0, sticky=tk.W, pady=(5, 0))
        self.track_condition = ttk.Combobox(race_frame, values=["Firm", "Good", "Yielding", "Heavy"], width=10)
        self.track_condition.grid(row=2, column=1, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        self.track_condition.set("Firm")
        
        ttk.Label(race_frame, text="Weather:").grid(row=2, column=2, sticky=tk.W, padx=(10, 0), pady=(5, 0))
        self.weather = ttk.Combobox(race_frame, values=["Clear", "Rain", "Snow"], width=10)
        self.weather.grid(row=2, column=3, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        self.weather.set("Clear")
        
        ttk.Label(race_frame, text="Race Grade:").grid(row=3, column=0, sticky=tk.W, pady=(5, 0))
        self.race_grade = ttk.Combobox(race_frame, values=["G1", "G2", "G3", "OP", "Debut"], width=10)
        self.race_grade.grid(row=3, column=1, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        self.race_grade.set("G1")
        
        ttk.Label(race_frame, text="Course Direction:").grid(row=3, column=2, sticky=tk.W, padx=(10, 0), pady=(5, 0))
        self.course_direction = ttk.Combobox(race_frame, values=["Clockwise", "Counterclockwise"], width=12)
        self.course_direction.grid(row=3, column=3, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        self.course_direction.set("Clockwise")
        ...
        # New advanced uma settings
        adv_frame = ttk.Frame(uma_frame)
        adv_frame.grid(row=1, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=(5, 0))
        
        ttk.Label(adv_frame, text="Form:").grid(row=0, column=0, sticky=tk.W)
        self.uma_form = ttk.Combobox(adv_frame, values=["Peak", "Good", "Normal", "Poor", "Tired"], width=8)
        self.uma_form.grid(row=0, column=1, sticky=tk.W, padx=(5, 0))
        self.uma_form.set("Normal")
        
        ttk.Label(adv_frame, text="Motivation:").grid(row=0, column=2, sticky=tk.W, padx=(10, 0))
        self.uma_motivation = ttk.Combobox(adv_frame, values=["Best", "Good", "Normal", "Bad", "Worst"], width=8)
        self.uma_motivation.grid(row=0, column=3, sticky=tk.W, padx=(5, 0))
        self.uma_motivation.set("Normal")
        
        ttk.Label(adv_frame, text="Equipment:").grid(row=0, column=4, sticky=tk.W, padx=(10, 0))
        self.uma_equipment = ttk.Combobox(adv_frame, values=["Standard", "Spiked", "Light", "Heavy"], width=8)
        self.uma_equipment.grid(row=0, column=5, sticky=tk.W, padx=(5, 0))
        self.uma_equipment.set("Standard")
        ...
        apt_frame.grid(row=3, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=(10, 0))
        ...
        # New advanced aptitudes
        ttk.Label(apt_frame, text="Strategy Aptitude:").grid(row=2, column=0, sticky=tk.W, pady=(5, 0))
        
        ttk.Label(apt_frame, text="Front:").grid(row=2, column=1, sticky=tk.W, padx=(10, 0), pady=(5, 0))
        self.apt_front = ttk.Combobox(apt_frame, values=["S", "A", "B", "C", "D", "E", "F", "G"], width=3)
        self.apt_front.grid(row=2, column=2, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        self.apt_front.set("B")
        
        ttk.Label(apt_frame, text="Stalk:").grid(row=2, column=3, sticky=tk.W, padx=(10, 0), pady=(5, 0))
        self.apt_stalk = ttk.Combobox(apt_frame, values=["S", "A", "B", "C", "D", "E", "F", "G"], width=3)
        self.apt_stalk.grid(row=2, column=4, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        self.apt_stalk.set("B")
        
        ttk.Label(apt_frame, text="Sustain:").grid(row=2, column=5, sticky=tk.W, padx=(10, 0), pady=(5, 0))
        self.apt_sustain = ttk.Combobox(apt_frame, values=["S", "A", "B", "C", "D", "E", "F", "G"], width=3)
        self.apt_sustain.grid(row=2, column=6, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        self.apt_sustain.set("B")
        
        ttk.Label(apt_frame, text="Close:").grid(row=2, column=7, sticky=tk.W, padx=(10, 0), pady=(5, 0))
        self.apt_close = ttk.Combobox(apt_frame, values=["S", "A", "B", "C", "D", "E", "F", "G"], width=3)
        self.apt_close.grid(row=2, column=8, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        self.apt_close.set("B")

        # Skills - Improved with category filtering
        skills_frame = ttk.LabelFrame(uma_frame, text="Skills", padding="5")
        skills_frame.grid(row=4, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=(10, 0))

        # Skill category filter
        ttk.Label(skills_frame, text="Filter by Category:").grid(row=0, column=0, sticky=tk.W)
        self.skill_category = ttk.Combobox(skills_frame, values=list(self.skill_categories.keys()), width=15)
        self.skill_category.grid(row=0, column=1, sticky=tk.W, padx=(5, 0))
        self.skill_category.set("All Skills")
        self.skill_category.bind('<<ComboboxSelected>>', self.filter_skills)

        ttk.Label(skills_frame, text="Search Skill:").grid(row=0, column=2, sticky=tk.W, padx=(10, 0))
        self.skill_search = ttk.Entry(skills_frame, width=20)
        self.skill_search.grid(row=0, column=3, sticky=tk.W, padx=(5, 0))
        self.skill_search.bind('<KeyRelease>', self.search_skills)

        # Skills listbox with scrollbar
        skills_list_frame = ttk.Frame(skills_frame)
        skills_list_frame.grid(row=1, column=0, columnspan=6, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(5, 0))

        self.skills_listbox = tk.Listbox(skills_list_frame, height=8)
        self.skills_scrollbar = ttk.Scrollbar(skills_list_frame, orient=tk.VERTICAL, command=self.skills_listbox.yview)
        self.skills_listbox.configure(yscrollcommand=self.skills_scrollbar.set)
        
        self.skills_listbox.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        self.skills_scrollbar.grid(row=0, column=1, sticky=(tk.N, tk.S))

        # Populate skills listbox
        self.populate_skills_listbox()

        # Skill management buttons
        ttk.Button(skills_frame, text="Add Selected Skill", command=self.add_selected_skill).grid(row=2, column=0, pady=(5, 0))
        ttk.Button(skills_frame, text="Remove Selected Skill", command=self.remove_selected_skill).grid(row=2, column=1, pady=(5, 0))

        # Current skills
        ttk.Label(skills_frame, text="Current Skills:").grid(row=3, column=0, sticky=tk.W, pady=(10, 0))
        self.current_skills_listbox = tk.Listbox(skills_frame, height=4)
        self.current_skills_listbox.grid(row=4, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=(5, 0))

        # Save/load buttons
        ttk.Button(uma_frame, text="Save Uma", command=self.save_uma).grid(row=5, column=0, pady=(10, 0))
        ttk.Button(uma_frame, text="Reset Form", command=self.reset_form).grid(row=5, column=1, pady=(10, 0))
        ...
        skills_list_frame.columnconfigure(0, weight=1)
        skills_list_frame.rowconfigure(0, weight=1)
        
        # Add some sample umas
        self.add_sample_umas()

    def populate_skills_listbox(self, skills_list=None):
        """Populate the skills listbox with skills"""
    ...
    def filter_skills(self, event=None):
        ...
        if category == "All Skills":
            self.populate_skills_listbox()
        elif category in self.skill_categories:
            self.populate_skills_listbox(self.skill_categories[category])

    def search_skills(self, event=None):
        """Search skills by name"""
        search_term = self.skill_search.get().lower()
        if not search_term:
            self.filter_skills()
            return
        
        filtered_skills = [skill for skill in self.skills if search_term in skill.lower()]
        self.populate_skills_listbox(filtered_skills)

    def add_selected_skill(self):
        """Add selected skill from listbox to current uma"""
        selection = self.skills_listbox.curselection()
        if selection and self.current_uma_index is not None:
            skill = self.skills_listbox.get(selection[0])
            uma = self.umas[self.current_uma_index]
            if skill not in uma["skills"]:
                uma["skills"].append(skill)
                self.update_current_skills_listbox()

    def remove_selected_skill(self):
        """Remove selected skill from current uma"""
        selection = self.current_skills_listbox.curselection()
        if selection and self.current_uma_index is not None:
            index = selection[0]
            uma = self.umas[self.current_uma_index]
            skill = self.current_skills_listbox.get(index)
            uma["skills"].remove(skill)
            self.update_current_skills_listbox()

    def update_current_skills_listbox(self):
        """Update the current skills listbox"""
        if self.current_uma_index is not None:
            uma = self.umas[self.current_uma_index]
            self.current_skills_listbox.delete(0, tk.END)
            for skill in uma.get("skills", []):
                self.current_skills_listbox.insert(tk.END, skill)
        ...
        sample_umas = [
            {
                ...
                "strategy_aptitude": {"Front": "A", "Stalk": "B", "Sustain": "B", "Close": "C"},
                "form": "Normal",
                "motivation": "Normal",
                "equipment": "Standard",
                "skills": ["#Beauty Sprint", "Acceleration", "Front Runner's Pride", "Golden Factor"]
            },
            {
                "name": "Prince Loupan",
                "running_style": "PC",
                ...
                "distance_aptitude": {"Sprint": "A", "Mile": "A", "Medium": "A", "Long": "A"},
                "surface_aptitude": {"Dirt": "A", "Turf": "A"},
                "strategy_aptitude": {"Front": "B", "Stalk": "A", "Sustain": "B", "Close": "B"},
                "form": "Normal",
                "motivation": "Normal",
                "equipment": "Standard",
                "skills": ["#Cute Perturbation♪", "Breakthrough", "Pace Chaser's Insight", "Timely Charge"]
            }
        ]
    ...
    def add_uma(self):
        ...
        new_uma = {
            ...
            "strategy_aptitude": {"Front": "B", "Stalk": "B", "Sustain": "B", "Close": "B"},
            "form": "Normal",
            "motivation": "Normal",
            "equipment": "Standard",
            "skills": []
    ...
    def on_uma_select(self, event=None):
        ...
        if selection:
            ...
            # Set advanced states
            self.uma_form.set(uma.get("form", "Normal"))
            self.uma_motivation.set(uma.get("motivation", "Normal"))
            self.uma_equipment.set(uma.get("equipment", "Standard"))
            ...
            # Set strategy aptitudes
            strat_apt = uma.get("strategy_aptitude", {})
            self.apt_front.set(strat_apt.get("Front", "B"))
            self.apt_stalk.set(strat_apt.get("Stalk", "B"))
            self.apt_sustain.set(strat_apt.get("Sustain", "B"))
            self.apt_close.set(strat_apt.get("Close", "B"))
    ...
    def save_uma(self):
        ...
        uma["form"] = self.uma_form.get()
        uma["motivation"] = self.uma_motivation.get()
        uma["equipment"] = self.uma_equipment.get()
        ...
        # Update strategy aptitudes
        uma["strategy_aptitude"] = {
            "Front": self.apt_front.get(),
            "Stalk": self.apt_stalk.get(),
            "Sustain": self.apt_sustain.get(),
            "Close": self.apt_close.get()
        }
    ...
    def reset_form(self):
        ...
        self.current_skills_listbox.delete(0, tk.END)
        self.skill_category.set("All Skills")
        self.skill_search.delete(0, tk.END)
        self.populate_skills_listbox()
    ...
    def generate_config(self):
        ...
        config = {
            "race": {
                ...
                "grade": self.race_grade.get(),
                "course_direction": self.course_direction.get(),
                "num_corners": 4,  # Default value
                "home_straight_length": 400  # Default value
            },
            "umas": self.umas
        }
    ...
    def load_from_file(self):
        ...
        if filename:
            try:
                ...
                self.track_condition.set(race.get("track_condition", "Firm"))
                self.weather.set(race.get("weather", "Clear"))
                self.race_grade.set(race.get("grade", "G1"))
                self.course_direction.set(race.get("course_direction", "Clockwise"))
