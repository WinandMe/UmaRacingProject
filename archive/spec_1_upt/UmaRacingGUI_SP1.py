...
from PySide6.QtWidgets import (
    ...
    QFrame, QMessageBox, QListWidget, QListWidgetItem, QScrollArea, QDialog
)
...
class RaceCanvasWidget(QWidget):
    ...
    def __init__(self):
        ...
        # Post-race animation
        self.finished_text_opacity = 0.0
        self.canvas_opacity = 1.0
        self.showing_honorable_mentions_overlay = False
        self.honorable_mention_scroll_offset = 0
        
        # Achievement data for honorable mentions overlay
        self.uma_of_race_horse = None
        self.horse_achievements = {}
        self.achievement_lines = []  # Pre-generated achievement lines
    ...
    def paintEvent(self, event):
        ...
        # DEBUG: Check if we're showing overlay
        if self.showing_honorable_mentions_overlay:
            print(f"PAINT EVENT: showing_honorable_mentions_overlay={self.showing_honorable_mentions_overlay}")
        ...
        # Draw canvas fade overlay for post-race
        if self.canvas_opacity < 1.0:
            painter.setOpacity(1.0 - self.canvas_opacity)
            painter.fillRect(0, 0, self.width(), self.height(), QColor('black'))
            painter.setOpacity(1.0)
        
        # Draw FINISHED text overlay
        if self.finished_text_opacity > 0:
            painter.setOpacity(self.finished_text_opacity)
            ...
            font.setBold(True)
            painter.setFont(font)
            painter.setPen(QPen(QColor('#FFD700')))  # Gold color
        ...
        # Draw honorable mentions overlay on canvas
        if self.showing_honorable_mentions_overlay:
            self.draw_honorable_mentions_overlay(painter)
    
    def draw_honorable_mentions_overlay(self, painter):
        """Draw honorable mentions on the canvas"""
        print(f"DEBUG draw_honorable_mentions called!")
        print(f"  achievement_lines length: {len(self.achievement_lines) if hasattr(self, 'achievement_lines') else 'NO ATTR'}")
        print(f"  uma_of_race_horse: {self.uma_of_race_horse if hasattr(self, 'uma_of_race_horse') else 'NO ATTR'}")
        ...
        painter.drawText(title_x, 50, title)
        
        # Uma of the Race section
        y_offset = 100
        if hasattr(self, 'uma_of_race_horse') and self.uma_of_race_horse:
            font.setPointSize(16)
            font.setBold(True)
            painter.setFont(font)
            painter.setPen(QPen(QColor('#FFC700')))
            painter.drawText(40, y_offset, "⭐ UMA OF THE RACE ⭐")
            
            font.setPointSize(14)
            painter.setFont(font)
            painter.setPen(QPen(QColor('#FFFFFF')))
            painter.drawText(60, y_offset + 35, self.uma_of_race_horse)
            y_offset += 55
            
            # Display Uma of the Race reason/commentary
            if hasattr(self, 'uma_of_race_reason') and self.uma_of_race_reason:
                font.setPointSize(10)
                font.setBold(False)
                painter.setFont(font)
                painter.setPen(QPen(QColor('#B0C4DE')))  # Light steel blue
                
                # Word wrap the reason text
                max_width = w - 120
                words = self.uma_of_race_reason.split()
                lines = []
                current_line = []
                ...
                for line in lines:
                    painter.drawText(60, y_offset, line)
                    y_offset += 18
                
            y_offset += 25
        
        # Individual achievements header
        font.setPointSize(12)
        font.setBold(True)
        painter.setFont(font)
        painter.setPen(QPen(QColor('#FFD700')))
        painter.drawText(40, y_offset, "FINAL STANDINGS")
        y_offset += 30
        ...
        line_height = 28
        max_lines = (h - y_offset - 40) // line_height
        
        print(f"  Drawing {len(self.achievement_lines) if hasattr(self, 'achievement_lines') else 0} achievement lines")
        print(f"  max_lines: {max_lines}, y_offset: {y_offset}, h: {h}")
        
        if hasattr(self, 'achievement_lines') and self.achievement_lines:
            for i, achievement_line in enumerate(self.achievement_lines):
                if i >= max_lines:
                    break
                ...
                if position == 1:
                    ...
                    painter.setPen(QPen(QColor('#CD7F32')))
                else:
                    painter.setPen(QPen(QColor('#CCCCCC')))
                
                print(f"    Drawing line {i}: {achievement_line[:50]}...")
                painter.drawText(50, y_offset, w - 100, 25, Qt.TextFlag.TextWordWrap, achievement_line)
                y_offset += line_height
        else:
            # Debug: show empty message
            painter.setPen(QPen(QColor('#FF6B6B')))
            font.setPointSize(16)
            painter.setFont(font)
            painter.drawText(50, h // 2, w - 100, 100, Qt.TextFlag.AlignCenter | Qt.TextFlag.TextWordWrap, "No achievement data loaded - Debug Info Available")
...
class UmaRacingGUI(QMainWindow):
    def __init__(self):
        ...
        # === POST-RACE ACHIEVEMENT SYSTEM ===
        self.race_finished = False
        self.post_race_phase = None  # 'finished_text', 'uma_of_race', 'standings_expand', 'ready_for_mentions'
        self.finished_text_start_time = 0.0  # Wall-clock time, not sim_time
        self.canvas_fade_start_time = 0.0
        self.finished_text_opacity = 0.0
        self.canvas_opacity = 1.0
        self.uma_of_race_horse = None
        self.uma_of_race_achievement = ""
        self.horse_achievements = {}  # Dict: horse_name -> achievement_text
        self.event_scores = {}  # Dict: horse_name -> score (overtakes, defends, etc.)
        self.showing_honorable_mentions = False
        self.showing_honorable_mentions_overlay = False
    ...
    def setup_ui(self):
        ...
        # Honorable mentions button (hidden until race finishes)
        self.honorable_mentions_btn = QPushButton("🏆 Honorable Mentions")
        self.honorable_mentions_btn.clicked.connect(self.show_honorable_mentions)
        self.honorable_mentions_btn.setEnabled(False)
        self.honorable_mentions_btn.setStyleSheet("""
            QPushButton {
        ...
        """)
        control_layout.addWidget(self.honorable_mentions_btn)
        ...
        # Header
        self.positions_header = QLabel("🏁 LIVE POSITIONS")
        self.positions_header.setStyleSheet("""
            QLabel {
                color: #ffffff;
                font-weight: bold;
                font-size: 11px;
        ...
        """)
        self.positions_header.setAlignment(Qt.AlignmentFlag.AlignCenter)
        positions_layout.addWidget(self.positions_header)
        ...
        """)
        self.positions_list.itemDoubleClicked.connect(self.show_participant_details)
    ...
    def run_simulation_frame(self):
        ...
        elif self.race_finished and self.post_race_phase:
            # Continue post-race animation even after sim stops
            self.update_post_race_animation()
    ...
    def _run_real_time_tick(self):
        ...
        try:
            ...
            if self.race_engine.is_finished:
                ...
                # Start post-race flow
                if not self.race_finished:
                    import time
                    self.race_finished = True
                    self.post_race_phase = 'finished_text'
                    self.finished_text_start_time = time.time()
                    self.compute_uma_of_race()
                    self.compute_horse_achievements()
                    self.update_display(current_frame_positions, race_distance)
                else:
                    # Continue post-race animation
                    self.update_post_race_animation()
    ...
    def update_display(self, frame_positions, race_distance):
        ...
        # Apply post-race animation opacities
        self.race_canvas.finished_text_opacity = self.finished_text_opacity
        self.race_canvas.canvas_opacity = self.canvas_opacity
    ...
    def display_final_results(self):
        ...
        # Change header to FINAL STANDINGS
        self.positions_header.setText("🏆 FINAL STANDINGS")
    ...
    def compute_uma_of_race(self):
        """Calculate which horse had the most impactful race based on events"""
        # Score horses based on race events
        scores = {}
        
        # Initialize all horses with 0
        ...
        # Finishing position: winner gets +50, 2nd gets +30, 3rd gets +15, others get their position reversed
        if self.finish_times:
        ...
        # Stamina sustainability: finishing with good stamina = +5
        for name in self.uma_icons.keys():
            if name in self.uma_finished and self.uma_finished[name]:
        ...
        # Find the horse with the highest score
        self.event_scores = scores
        self.uma_of_race_horse = max(scores.keys(), key=lambda x: scores[x])
        
        # Generate Uma of the Race commentary
        self.uma_of_race_reason = self.get_uma_of_race_commentary(self.uma_of_race_horse)
    
    def get_uma_of_race_commentary(self, uma_name: str) -> str:
        """Generate creative commentary explaining why this Uma earned 'Uma of the Race'"""
        if not uma_name or uma_name not in self.uma_icons:
            return "Outstanding performance throughout the race!"
        ...
        start_pos = int(self.gate_numbers.get(uma_name, 1))
        position_change = start_pos - final_position
        stamina_remaining = self.uma_stamina.get(uma_name, 50)
        
        # Get uma stats if available
        uma_stats = self.sim_data.get('uma_stats', {}).get(uma_name, {})
        running_style = uma_stats.get('running_style', 'Unknown')
        
        # Create a hash to consistently select the same reason for the same performance
        performance_hash = hash(f"{uma_name}{final_position}{overtakes}{position_change}")
        
        # 100+ Reason templates categorized by performance type
        reasons = []
        
        # === DOMINANT VICTORY REASONS (for winners) ===
        if final_position == 1:
            reasons.extend([
                f"Commanded the race from start to finish with unwavering authority!",
                f"Delivered a masterclass in racing strategy and execution!",
                f"Showed champion-level composure under maximum pressure!",
                f"Executed a textbook race with surgical precision!",
                f"Dominated the field with breathtaking speed and control!",
                f"Displayed world-class racing intelligence throughout!",
                f"Set the pace and never looked back - a true champion's performance!",
                f"Combined raw speed with tactical brilliance for a stunning victory!",
                f"Made it look easy with a performance for the ages!",
                f"Answered every challenge with unwavering determination!",
                f"Proved why they're among the elite with this commanding win!",
                f"Turned the race into a personal showcase of talent!"
            ])
        
        # === OVERTAKING EXCELLENCE ===
        if overtakes >= 5:
            reasons.extend([
                f"Carved through the field with {overtakes} spectacular overtakes!",
                f"Demonstrated incredible overtaking prowess with {overtakes} decisive moves!",
                f"Showed relentless attacking spirit with {overtakes} position gains!",
                f"Executed {overtakes} surgical overtakes that changed the race!",
                f"Made {overtakes} bold moves that left rivals in their wake!"
            ])
        elif overtakes >= 3:
            reasons.extend([
                f"Timed {overtakes} crucial overtakes with perfect precision!",
                f"Made {overtakes} strategic moves that defined their race!",
                f"Displayed excellent racecraft with {overtakes} key overtakes!",
                f"Found gaps others missed with {overtakes} smart maneuvers!"
            ])
        elif overtakes >= 1:
            reasons.extend([
                f"Executed decisive overtakes at critical moments!",
                f"Made every position count with well-timed moves!",
                f"Showed patience before striking with precision!"
            ])
        
        # === COMEBACK HEROICS ===
        if position_change >= 6:
            reasons.extend([
                f"Mounted an incredible comeback from gate {start_pos} to finish {final_position}!",
                f"Overcame a {position_change}-position deficit with breathtaking determination!",
                f"Transformed a challenging start into a spectacular performance!",
                f"Proved that starting position is no barrier to greatness!",
                f"Fought from the back to deliver an unforgettable charge!"
            ])
        elif position_change >= 3:
            reasons.extend([
                f"Steadily carved through the field gaining {position_change} positions!",
                f"Showed tactical patience before unleashing a {position_change}-position surge!",
                f"Demonstrated classic race management moving up {position_change} spots!",
                f"Built momentum perfectly to advance {position_change} positions!"
            ])
        
        # === STAMINA MASTERY ===
        if stamina_remaining > 70:
            reasons.extend([
                f"Finished with exceptional {stamina_remaining:.0f}% stamina - a testament to perfect pacing!",
                f"Managed energy reserves brilliantly, crossing with {stamina_remaining:.0f}% stamina remaining!",
                f"Demonstrated superior conditioning with {stamina_remaining:.0f}% stamina at the line!",
                f"Showed textbook stamina management throughout the entire race!",
                f"Never looked tired, finishing with abundant reserves!"
            ])
        elif stamina_remaining < 20:
            reasons.extend([
                f"Gave absolutely everything, pushing to the absolute limit!",
                f"Showed incredible heart crossing the line on pure willpower!",
                f"Left nothing in reserve - a warrior's performance!",
                f"Demonstrated the true meaning of determination and guts!",
                f"Ran on empty but refused to give up - pure champion mentality!"
            ])
        
        # === RUNNING STYLE EXCELLENCE ===
        if running_style == 'FR':
            reasons.extend([
                f"Executed a perfect front-running strategy from gate to wire!",
                f"Set a blistering pace that broke the field's spirit!",
                f"Led with confidence and never looked back!",
                f"Dictated the race tempo with masterful front-running!"
            ])
        elif running_style == 'PC':
            reasons.extend([
                f"Found the perfect stalking position and struck at the ideal moment!",
                f"Demonstrated textbook pace-chasing with surgical precision!",
                f"Sat in the perfect spot before unleashing their finishing kick!",
                f"Showed patience and timing that defines great pace chasers!"
            ])
        elif running_style == 'LS':
            reasons.extend([
                f"Timed their late surge to perfection!",
                f"Showed why late surgers are the most dangerous closers!",
                f"Made their move exactly when it mattered most!",
                f"Demonstrated classic late-surging tactics with devastating effect!"
            ])
        elif running_style == 'EC':
            reasons.extend([
                f"Closed from the back with breathtaking acceleration!",
                f"Proved that closers can dominate with the right execution!",
                f"Made their closing kick count when it mattered most!",
                f"Showed elite closing speed in the final stretch!"
            ])
        
        # === TACTICAL BRILLIANCE ===
        reasons.extend([
            f"Read the race perfectly and positioned themselves ideally!",
            f"Demonstrated world-class racing intelligence throughout!",
            f"Made all the right decisions at all the right times!",
            f"Showed tactical awareness that separated them from the field!",
            f"Executed their race plan to absolute perfection!",
            f"Navigated traffic with the skill of a seasoned champion!",
            f"Found racing room where others saw only dead ends!",
            f"Anticipated every move and countered with precision!"
        ])
        
        # === SPEED AND POWER ===
        reasons.extend([
            f"Unleashed devastating speed when it mattered most!",
            f"Combined raw power with finesse for an outstanding display!",
            f"Showed acceleration that left rivals gasping!",
            f"Demonstrated elite-level speed throughout the race!",
            f"Powered through the finish with authority!",
            f"Made their speed advantage count in spectacular fashion!"
        ])
        
        # === MENTAL STRENGTH ===
        reasons.extend([
            f"Showed nerves of steel under intense pressure!",
            f"Never panicked, always composed - a champion's mindset!",
            f"Demonstrated mental toughness that defines winners!",
            f"Kept their cool when the heat was on!",
            f"Showed the heart of a lion in the heat of battle!",
            f"Refused to be intimidated by their rivals!"
        ])
        
        # === CONSISTENCY AND RELIABILITY ===
        reasons.extend([
            f"Delivered consistent performance from start to finish!",
            f"Never missed a beat throughout the entire race!",
            f"Showed reliability that coaches dream about!",
            f"Maintained form when others were fading!",
            f"Proved their class with steady, measured racing!"
        ])
        
        # === CLUTCH PERFORMANCE ===
        reasons.extend([
            f"Delivered when it mattered most - true clutch performance!",
            f"Raised their game at the crucial moment!",
            f"Showed up when the pressure was at its peak!",
            f"Made the big moments count with champion-level execution!",
            f"Proved they're built for the biggest stages!"
        ])
        
        # === RACING ARTISTRY ===
        reasons.extend([
            f"Turned racing into an art form with this performance!",
            f"Combined grace and power in perfect harmony!",
            f"Made every stride look effortless and beautiful!",
            f"Showed why racing can be poetry in motion!",
            f"Delivered a performance that will be remembered for years!"
        ])
        
        # === DETERMINATION AND GRIT ===
        reasons.extend([
            f"Fought for every meter with unwavering determination!",
            f"Showed guts and determination that inspired everyone watching!",
            f"Never gave up, never gave in - pure fighting spirit!",
            f"Battled through adversity with incredible resilience!",
            f"Demonstrated the never-say-die attitude of a champion!"
        ])
        
        # === SPECIAL MENTIONS (Position-specific) ===
        if final_position == 2:
            reasons.extend([
                f"Pushed the winner all the way - an outstanding effort!",
                f"So close to victory but gave everything in pursuit!",
                f"Made the winner earn it with relentless pressure!",
                f"Delivered a performance worthy of celebration despite finishing second!"
            ])
        elif final_position == 3:
            reasons.extend([
                f"Secured a podium finish through sheer determination!",
                f"Showed consistent strength to claim third!",
                f"Made the podium their own with solid racing!"
            ])
        elif final_position <= 5:
            reasons.extend([
                f"Finished strongly in the top five with impressive racing!",
                f"Showed they belong with the best with this performance!",
                f"Proved their quality with a top-five finish!"
            ])
        
        # === GENERAL EXCELLENCE ===
        reasons.extend([
            f"Set the standard for how the race should be run!",
            f"Showed class and quality that stood above the rest!",
            f"Earned this award with an all-around complete performance!",
            f"Demonstrated excellence in every phase of the race!",
            f"Made their mark with a truly outstanding display!",
            f"Showed why they deserve recognition with this effort!",
            f"Delivered a performance that embodied racing excellence!",
            f"Rose to the occasion in spectacular fashion!",
            f"Put on a show that won't soon be forgotten!",
            f"Earned every bit of recognition with this race!"
        ])
        
        # Select reason based on performance hash
        if reasons:
            selected_reason = reasons[abs(performance_hash) % len(reasons)]
            return selected_reason
        
        return "Delivered an exceptional all-around performance!"
    
    def compute_horse_achievements(self):
        """Generate achievement text for each horse"""
        self.horse_achievements = {}
        ...
        for name, finish_time in finished:
            achievement = self.generate_horse_achievement(name, finish_time)
            self.horse_achievements[name] = achievement
    
    def generate_horse_achievement(self, name: str, finish_time: float) -> str:
        """Generate an achievement text for a horse"""
        achievements = []
        
        # Get position
        finished = sorted(self.finish_times.items(), key=lambda x: x[1])
        position = next(i + 1 for i, (n, _) in enumerate(finished) if n == name)
        
        # Count overtakes by this horse
        overtakes_by_horse = sum(1 for o, _, _, _ in self.overtakes if o == name)
        
        # Position change from start (estimated gate number as start position)
        start_pos = int(self.gate_numbers.get(name, 1))
        position_change = start_pos - position
        
        # Generate achievement based on performance
        if position == 1:
            achievements.append(f"{name} crossed the finish line in triumph, claiming victory!")
        elif position == 2:
            achievements.append(f"{name} put up a valiant effort, securing second place!")
        elif position == 3:
            achievements.append(f"{name} made a strong showing in third place!")
        else:
            achievements.append(f"{name} finished in position {position}.")
        
        if position_change > 0:
            achievements.append(f"Moved forward {position_change} positions during the race.")
        elif position_change < 0:
            achievements.append(f"Pushed back {abs(position_change)} positions, fighting hard to maintain position.")
        
        if overtakes_by_horse > 0:
            achievements.append(f"Executed {overtakes_by_horse} successful overtake{'s' if overtakes_by_horse > 1 else ''}!")
        
        stamina_remaining = self.uma_stamina.get(name, 0)
        if stamina_remaining > 70:
            achievements.append("Crossed the line with excellent stamina remaining!")
        elif stamina_remaining < 20:
            achievements.append("Ran on fumes, giving everything to the finish line!")
        
        return " ".join(achievements)
    
    def update_post_race_animation(self):
        """Update post-race animation phases using wall-clock time"""
        import time
        current_time = time.time()
        
        if self.post_race_phase == 'finished_text':
            # Phase 1: FINISHED text fades in/out (5 seconds)
            elapsed = current_time - self.finished_text_start_time
            
            if elapsed < 5.0:
                # Fade in first 1 second
                if elapsed < 1.0:
                    self.finished_text_opacity = (elapsed / 1.0) * 0.8
                # Hold for 3 seconds
                elif elapsed < 4.0:
                    self.finished_text_opacity = 0.8
                # Fade out last 1 second
                else:
                    self.finished_text_opacity = 0.8 * (1.0 - (elapsed - 4.0) / 1.0)
                    
                # Keep showing final positions during animation
                final_positions = [(name, self.uma_distances.get(name, 0)) for name in self.finish_times.keys()]
                self.update_display(final_positions, self.race_engine.race_distance if self.race_engine else 0)
            else:
                # Transition to canvas fade-out phase
                self.post_race_phase = 'canvas_fade'
                self.canvas_fade_start_time = current_time
                self.finished_text_opacity = 0.0
        
        elif self.post_race_phase == 'canvas_fade':
            # Phase 2: Canvas fades out over 2 seconds, standings expand to full width
            fade_elapsed = current_time - self.canvas_fade_start_time
            if fade_elapsed < 2.0:
                self.canvas_opacity = 1.0 - (fade_elapsed / 2.0)
                # Expand positions frame
                expansion_factor = fade_elapsed / 2.0
                self.canvas_frame.setMaximumHeight(int(440 * (1.0 - expansion_factor * 0.5)))
            else:
                self.canvas_opacity = 0.0
                self.post_race_phase = 'uma_of_race'
                self.display_final_results()
                self.canvas_frame.hide()
                self.positions_frame.setMaximumWidth(99999)
                self.honorable_mentions_btn.setEnabled(True)
            
            # Keep showing final positions during fade
            final_positions = [(name, self.uma_distances.get(name, 0)) for name in self.finish_times.keys()]
            self.update_display(final_positions, self.race_engine.race_distance if self.race_engine else 0)
    
    def show_honorable_mentions(self):
        """Display honorable mentions overlay on canvas"""
        print(f"\n=== HONORABLE MENTIONS BUTTON CLICKED ===")
        print(f"Currently showing overlay: {self.showing_honorable_mentions_overlay}")
        print(f"Finish times available: {len(self.finish_times)} horses")
        print(f"Uma of race: {self.uma_of_race_horse}")
        
        if self.showing_honorable_mentions_overlay:
            # Switch back to final results
            print("Switching back to final results")
            self.showing_honorable_mentions_overlay = False
            self.race_canvas.showing_honorable_mentions_overlay = False
            
            # Hide canvas frame and show results panels
            self.canvas_frame.hide()
            self.positions_frame.show()
            self.output_text.show()
            self.output_text.clear()
            self.display_final_results()
            self.honorable_mentions_btn.setText("🏆 Honorable Mentions")
            self.race_canvas.update()
        else:
            # Show honorable mentions on canvas
            print("Showing honorable mentions overlay")
            self.showing_honorable_mentions_overlay = True
            self.race_canvas.showing_honorable_mentions_overlay = True
            
            # Generate achievement lines from final race results
            achievement_lines = []
            if self.finish_times:
                # Sort by finish time to get final standings
                finished = sorted(self.finish_times.items(), key=lambda x: x[1])
                print(f"Generated {len(finished)} finished umas")
                for i, (name, _) in enumerate(finished):
                    position = i + 1
                    achievement_line = self.get_position_specific_achievement(name, position, len(finished))
                    achievement_lines.append(achievement_line)
                    print(f"  {achievement_line}")
            else:
                print("No finish times available!")
            
            print(f"Total achievement lines: {len(achievement_lines)}")
            self.race_canvas.achievement_lines = achievement_lines
            self.race_canvas.uma_of_race_horse = self.uma_of_race_horse
            self.race_canvas.uma_of_race_reason = getattr(self, 'uma_of_race_reason', '')
            
            # Hide both output and positions frame to show full canvas
            print("Hiding positions_frame and output_text")
            self.output_text.hide()
            self.positions_frame.hide()
            
            # Ensure canvas is shown and takes full space
            self.canvas_frame.show()
            self.race_canvas.show()
            
            # Force the layout to update
            self.race_canvas.setFocus()
            
            print(f"Canvas size before update: {self.race_canvas.width()}x{self.race_canvas.height()}")
            self.race_canvas.update()
            print(f"Canvas size after update: {self.race_canvas.width()}x{self.race_canvas.height()}")
            
            self.honorable_mentions_btn.setText("← Back to Results")
    
    def get_tazuna_advice(self, uma_name: str, stats: dict, running_style: str) -> str:
        """Generate Tazuna's humorous advice based on stats and running style"""
        # Extract key stats
        speed = stats.get('Speed', 0)
        stamina = stats.get('Stamina', 0)
        power = stats.get('Power', 0)
        ...
        # Define what each running style needs most
        style_priorities = {
            'FR': ('Speed', 'Guts', "Front Runners need blazing speed to lead from the start and guts to hold onto that lead!"),
            'PC': ('Speed', 'Power', "Pace Chasers must be quick enough to stay close and powerful enough to strike when it counts!"),
            'LS': ('Power', 'Stamina', "Late Surgers need raw power for that devastating final kick and stamina to make it to the closing stages!"),
            'EC': ('Power', 'Speed', "End Closers rely on power for their explosive closing run and speed to catch up from behind!")
        }
        
        primary_stat, secondary_stat, style_description = style_priorities.get(running_style, ('Speed', 'Stamina', "Train your horse well!"))
        
        # Get actual values for priority stats
        primary_value = stats.get(primary_stat, 0)
        secondary_value = stats.get(secondary_stat, 0)
        
        # Generate roasts and compliments based on stats
        advice_lines = []
        
        # ===== RUNNING STYLE SPECIFIC MAIN COMMENT =====
        if running_style == 'FR':
            if speed >= 1100:
                advice_lines.append("🔥 WOW! Your Front Runner's got JETS! That speed is absolutely insane - you've got a legitimate speed demon here!")
            elif speed >= 900:
                advice_lines.append("⚡ Nice speed for a Front Runner! Could be faster though. Your horse won't leave everyone in the dust with this.")
            else:
                advice_lines.append("😬 Oof... Your Front Runner's speed is kinda mid. Front Runners NEED that blazing start or they're toast.")
                
        elif running_style == 'PC':
            if speed >= 1000 and power >= 600:
                advice_lines.append("💪 Excellent choice! Your Pace Chaser has the right combo of speed AND power to strike at the perfect moment!")
            elif speed >= 900:
                advice_lines.append("👍 Your Pace Chaser's speed is solid. The timing will be everything in close races.")
            else:
                advice_lines.append("🤔 Your Pace Chaser lacks the speed to follow the leader effectively. Time to boost training!")
                
        elif running_style == 'LS':
            if power >= 700 and stamina >= 700:
                advice_lines.append("🚀 WHOA! Your Late Surger is an absolute MONSTER! That power + stamina combo will devastate opponents in the final stretch!")
            elif power >= 600:
                advice_lines.append("💥 Solid power levels! Your Late Surger can definitely deliver in the closing stages.")
            else:
                advice_lines.append("😅 Your Late Surger's power is... underwhelming. They'll get gassed before they even mount their charge!")
                
        elif running_style == 'EC':
            if power >= 700:
                advice_lines.append("⚡ Incredible power for an End Closer! You've got the raw firepower to shock everyone in that final sprint!")
            elif power >= 600:
                advice_lines.append("✨ Decent closing power! Your horse can definitely catch some slower opponents off guard.")
            else:
                advice_lines.append("😬 Your End Closer's power is weak. If you can't catch up, what's the point of being a closer?")
        
        # ===== STAMINA & GUTS COMMENTARY =====
        if stamina < 500:
            advice_lines.append(f"⚠️ Stamina crisis! At {stamina}, your trainee will be GASPING for air before halfway. Next time, try to crank the Stamina into ~700+ to make it decent.")
        elif stamina < 650:
            advice_lines.append(f"📊 Stamina is okay at {stamina}, but still needs work. Aim for ~700 to have a comfortable buffer.")
        elif stamina >= 800:
            advice_lines.append(f"💪 EXCELLENT stamina at {stamina}! Your horse can sustain an intense pace without collapsing!")
            
        if guts < 400:
            advice_lines.append(f"😰 Your trainee's willpower is non-existent ({guts}). Boost Guts to ~500+ so they don't give up when the pressure's on!")
        elif guts >= 550:
            advice_lines.append(f"🔥 Nice fighting spirit! {guts} Guts means your horse won't quit even when facing tough opponents!")
        
        # ===== WISDOM & SPEED BONUS COMMENTARY =====
        if wisdom >= 600:
            advice_lines.append(f"🧠 Your trainee's got brains! {wisdom} Wisdom will help them read the race and make smart tactical decisions!")
        
        if speed >= 1150:
            advice_lines.append(f"⚡ CRAZY FAST! {speed} speed is elite-tier. Your horse is a speed machine!")
        elif speed >= 1000:
            advice_lines.append(f"✨ Solid speed at {speed}. Not breaking records but competitive enough.")
        
        # ===== OVERALL POTENTIAL JUDGMENT =====
        total_stats = speed + stamina + power + guts + wisdom
        if total_stats >= 4500:
            advice_lines.append("🏆 CHAMPIONSHIP MATERIAL! Your trainee is stacked across the board. This horse is ready to dominate!")
        elif total_stats >= 4000:
            advice_lines.append("🌟 Pretty solid all-around! Your horse has real potential to compete at the highest level!")
        elif total_stats >= 3500:
            advice_lines.append("👍 Decent stats overall. With the right race strategy, your horse can pull off upsets!")
        else:
            advice_lines.append("📚 Your trainee needs more well-rounded development. Boost those overall stats!")
        
        return "\n\n".join(advice_lines)
    
    def show_participant_details(self, item):
        """Show detailed stats dialog when clicking a participant in standings"""
        print(f"\n=== PARTICIPANT DETAILS CLICKED ===")
        print(f"Item text: '{item.text()}'")
        
        if not self.config_data:
            print("ERROR: No config_data available")
            return
        
        # Extract uma name from the item text (remove position number, gate, emoji, etc)
        item_text = item.text()
        # Format is like: " 5: Tanino Gimlet (G9) +1.90s 🏁" or " 1: Tamamo Cross (G18) 🏁"
        # Extract name between colon and gate number
        try:
            # Split by ':' to get after position number
            if ':' in item_text:
                after_colon = item_text.split(':', 1)[1].strip()
                print(f"After colon split: '{after_colon}'")
                # Split by '(' to get name before gate number
                uma_name = after_colon.split('(')[0].strip()
                print(f"Extracted uma name: '{uma_name}'")
            else:
                print("ERROR: No colon found in item text")
                return
        except Exception as e:
            print(f"ERROR extracting name: {e}")
            return
        
        # Find uma data in config
        print(f"Searching for uma in {len(self.config_data.get('umas', []))} umas")
        uma_data = None
        for uma in self.config_data.get('umas', []):
            if uma.get('name') == uma_name:
                uma_data = uma
                print(f"Found uma data for: {uma_name}")
                break
        
        if not uma_data:
            print(f"ERROR: Uma '{uma_name}' not found in config")
            print(f"Available umas: {[u.get('name') for u in self.config_data.get('umas', [])]}")
            return
        
        print(f"Creating dialog for {uma_name}")
        
        # Create detail dialog
        dialog = QDialog(self)
        dialog.setWindowTitle(f"📊 {uma_name} - Detailed Stats")
        ...
        # Header with name and running style
        header = QLabel(f"<h2 style='color: #FFD700;'>{uma_name}</h2>")
        header.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(header)
        
        running_style = uma_data.get('running_style', 'N/A')
        style_names = {'FR': 'Front Runner', 'PC': 'Pace Chaser', 'LS': 'Late Surger', 'EC': 'End Closer'}
        style_label = QLabel(f"<h3 style='color: #90CAF9;'>Running Style: {style_names.get(running_style, running_style)}</h3>")
        style_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(style_label)
        
        # Stats section
        stats = uma_data.get('stats', {})
        stats_text = QLabel("<h3 style='color: #FFD700;'>📈 Base Stats</h3>")
        layout.addWidget(stats_text)
        
        stats_grid = QLabel(f"""
            <table style='color: #ffffff; width: 100%;'>
                <tr>
                    <td><b>⚡ Speed:</b></td><td style='text-align: right;'>{stats.get('Speed', 0)}</td>
                    <td width='50'></td>
                    <td><b>🔥 Power:</b></td><td style='text-align: right;'>{stats.get('Power', 0)}</td>
                </tr>
                <tr>
                    <td><b>💪 Stamina:</b></td><td style='text-align: right;'>{stats.get('Stamina', 0)}</td>
                    <td></td>
                    <td><b>🎯 Guts:</b></td><td style='text-align: right;'>{stats.get('Guts', 0)}</td>
                </tr>
                <tr>
                    <td><b>🧠 Wisdom:</b></td><td style='text-align: right;'>{stats.get('Wit', 0)}</td>
                    <td></td>
                    <td></td><td></td>
                </tr>
            </table>
        """)
        stats_grid.setStyleSheet("padding: 10px; background-color: #16213e; border-radius: 5px;")
        layout.addWidget(stats_grid)
        
        # Distance aptitudes
        dist_apt = uma_data.get('distance_aptitude', {})
        apt_label = QLabel("<h3 style='color: #FFD700;'>🏃 Distance Aptitudes</h3>")
        layout.addWidget(apt_label)
        
        def get_apt_color(apt):
            colors = {'S': '#FFD700', 'A': '#90EE90', 'B': '#87CEEB', 'C': '#FFA500', 
                     'D': '#FF6347', 'E': '#DC143C', 'F': '#8B0000', 'G': '#4B0000'}
            return colors.get(apt, '#CCCCCC')
        
        dist_text = QLabel(f"""
            <table style='width: 100%;'>
                <tr>
                    <td><b>Sprint (Short):</b></td>
                    <td style='text-align: right; color: {get_apt_color(dist_apt.get("Sprint", "C"))};'><b>{dist_apt.get('Sprint', 'C')}</b></td>
                    <td width='50'></td>
                    <td><b>Mile (Medium-Short):</b></td>
                    <td style='text-align: right; color: {get_apt_color(dist_apt.get("Mile", "C"))};'><b>{dist_apt.get('Mile', 'C')}</b></td>
                </tr>
                <tr>
                    <td><b>Medium (Middle):</b></td>
                    <td style='text-align: right; color: {get_apt_color(dist_apt.get("Medium", "C"))};'><b>{dist_apt.get('Medium', 'C')}</b></td>
                    <td></td>
                    <td><b>Long (Distance):</b></td>
                    <td style='text-align: right; color: {get_apt_color(dist_apt.get("Long", "C"))};'><b>{dist_apt.get('Long', 'C')}</b></td>
                </tr>
            </table>
        """)
        dist_text.setStyleSheet("padding: 10px; background-color: #16213e; border-radius: 5px;")
        layout.addWidget(dist_text)
        
        # Surface aptitudes
        surf_apt = uma_data.get('surface_aptitude', {})
        surf_label = QLabel("<h3 style='color: #FFD700;'>🌿 Surface Aptitudes</h3>")
        ...
        surf_text = QLabel(f"""
            <table style='width: 100%;'>
                <tr>
                    <td><b>🌱 Turf:</b></td>
                    <td style='text-align: right; color: {get_apt_color(surf_apt.get("Turf", "C"))};'><b>{surf_apt.get('Turf', 'C')}</b></td>
                    <td width='50'></td>
                    <td><b>🏜️ Dirt:</b></td>
                    <td style='text-align: right; color: {get_apt_color(surf_apt.get("Dirt", "C"))};'><b>{surf_apt.get('Dirt', 'C')}</b></td>
                </tr>
            </table>
        """)
        surf_text.setStyleSheet("padding: 10px; background-color: #16213e; border-radius: 5px;")
        layout.addWidget(surf_text)
        
        # Skills section (if available)
        skills = uma_data.get('skills', [])
        if skills:
            skills_label = QLabel(f"<h3 style='color: #FFD700;'>✨ Skills ({len(skills)})</h3>")
            layout.addWidget(skills_label)
            
            skills_list = QTextEdit()
            skills_list.setReadOnly(True)
            skills_list.setMaximumHeight(150)
            skills_list.setStyleSheet("""
                background-color: #16213e; 
                color: #ffffff; 
                border-radius: 5px;
                padding: 5px;
            ...
            for i, skill in enumerate(skills, 1):
                if isinstance(skill, dict):
                    ...
                    skill_name = skill
                skills_text += f"{i}. {skill_name}\n"
            
            skills_list.setText(skills_text)
            layout.addWidget(skills_list)
        else:
            no_skills = QLabel("<p style='color: #888888;'>No skills data available</p>")
            no_skills.setAlignment(Qt.AlignmentFlag.AlignCenter)
            layout.addWidget(no_skills)
        
        # Tazuna's Advice section
        advice = self.get_tazuna_advice(uma_name, stats, running_style)
        advice_label = QLabel("<h3 style='color: #FF6B9D;'>🎓 Tazuna's Advice</h3>")
        layout.addWidget(advice_label)
        
        advice_text = QTextEdit()
        advice_text.setReadOnly(True)
        advice_text.setMaximumHeight(250)
        ...
        print(f"Showing dialog for {uma_name}")
        dialog.exec()
        print(f"Dialog closed for {uma_name}")
    
    def get_creative_achievement_lines(self):
        """Generate 100+ creative, varied achievement lines for horses"""
        base_lines = [
            # Victory themes
            "crossed the finish line in a blaze of glory!",
            "claimed victory with determination burning bright!",
            "surged ahead to seize first place!",
            "dominated the course with masterful control!",
            "captured the crown with an explosive finish!",
            "triumphed over all challengers!",
            "secured the win with pure heart and speed!",
            "proved they are the champion today!",
            
            # Strong finishes
            "delivered a powerful performance from start to finish!",
            "showed incredible drive throughout the race!",
            "maintained composure and clinched a strong result!",
            "fought hard every step of the way!",
            "displayed exceptional grit and determination!",
            "battled fiercely against tough competition!",
            "showcased remarkable racing prowess!",
            "demonstrated unwavering focus and dedication!",
            
            # Overtaking/Position changes
            "orchestrated a brilliant passing maneuver!",
            "threaded through traffic with surgical precision!",
            "executed flawless overtakes throughout!",
            "carved their way through the field!",
            "displayed masterful tactical racing!",
            "made bold, decisive moves when it mattered!",
            "showed fearless aggression when needed!",
            "demonstrated expert racecraft and timing!",
            
            # Stamina/Endurance themes
            "powered through to the finish with energy to spare!",
            "maintained incredible pace throughout the race!",
            "showed limitless stamina and vitality!",
            "never wavered despite the brutal distance!",
            "finished strong despite tremendous pressure!",
            "summoned strength from deep within!",
            "refused to fade when others faltered!",
            "demonstrated iron-willed perseverance!",
            
            # Team player/Supporting themes
            "contributed immensely to the team effort!",
            "proved to be a valuable presence on the track!",
            "played a key role in the team's success!",
            "demonstrated excellent racing sense!",
            "provided tactical support throughout!",
            "was a stabilizing force on the track!",
            
            # Underdog/Comeback themes
            "defied the odds to deliver an upset!",
            "staged an incredible comeback from behind!",
            "proved the doubters wrong with class!",
            "rose to the occasion when it mattered most!",
            "showed that heart beats experience!",
            "overcame early challenges admirably!",
            "bounced back from a difficult start!",
            "turned adversity into triumph!",
            
            # Smooth/Clean racing
            "executed a textbook perfect race!",
            "displayed racing at its finest!",
            "maintained perfect rhythm throughout!",
            "showed flawless execution and pacing!",
            "raced with grace and confidence!",
            "demonstrated pristine racecraft!",
            "never put a hoof wrong all day!",
            "ran a clean, efficient race from start to finish!",
            
            # Explosive finishes
            "unleashed a devastating final sprint!",
            "delivered a knockout punch at the finish!",
            "exploded forward in the closing stages!",
            "tore through the final stretch with intensity!",
            "summoned a mighty finishing kick!",
            "launched a thrilling last-minute charge!",
            "showed their true class in the final furlong!",
            "saved the best for last with style!",
            
            # Determination/Will to win
            "wanted victory more than anyone!",
            "raced with unquenchable desire!",
            "showed championship mentality!",
            "displayed a winner's instinct!",
            "never gave up despite tough odds!",
            "proved their competitive fire burns bright!",
            "raced with pride and honor!",
            "showed what true competitors are made of!",
            
            # Agility/Skill themes
            "showed nimble footwork around the track!",
            "navigated the course with balletic grace!",
            "demonstrated exceptional turning ability!",
            "showed incredible corner technique!",
            "proved their handling is world-class!",
            "displayed cat-like reflexes and movement!",
            "showed supremely confident control!",
            "moved with grace under pressure!",
        ]
        return base_lines
    
    def get_position_specific_achievement(self, name: str, position: int, total: int) -> str:
        """Get a creative achievement line specific to their finishing position"""
        lines = self.get_creative_achievement_lines()
        
        # Get position-specific prefix
        if position == 1:
            prefix_lines = [
                "🥇 ", "⭐ ", "👑 ", "🏆 "
            ]
            prefix = prefix_lines[hash(name) % len(prefix_lines)]
        elif position == 2:
            prefix_lines = [
                "🥈 ", "💫 ", "✨ "
            ]
        ...
        # Select achievement based on hash for consistency
        achievement = lines[hash(name + str(position)) % len(lines)]
        return f"{prefix}{name} {achievement}"
